...
string HybridParallelHash::digest(const vector<uint8_t> &message, size_t L)
{
    ...
    size_t words_needed = L / 32;
    vector<uint32_t> D1, D2;
    finalize_and_fill_matrix(final_state_1, D1, words_needed, base_r1, base_r2);
    finalize_and_fill_matrix(final_state_2, D2, words_needed, base_r1, base_r2);

    // now D1.size() == D2.size() == words_needed
    vector<uint8_t> out_bytes_vec = combine_matrices(D1, D2, L / 8);

    return to_hex_digest(out_bytes_vec);
}
...
HybridParallelHash::State HybridParallelHash::derive_initial_state(const vector<uint32_t> &blocks) const
{
    ...
    if (N == 0)
        return State{0.5, 0.5, 0.5}; // Handle empty padded msg
...
HybridParallelHash::State HybridParallelHash::transient_warmup(const State &init, uint32_t iters) const
{
    ...
    for (uint32_t i = 0; i < iters; i++)
    {
        ...
        if ((i & 31) == 0)
            s = recombine(s, r1_local, r2_local);
    }
    return s;
}
...
// -------------------------
// Replacement: regen_r_from_uint (integer-state + mapping, splitmix64 mixing)
// -------------------------
static inline uint64_t splitmix64(uint64_t &x)
{
    // standard splitmix64: returns a mixed 64-bit value and advances the state
    // We'll not advance the caller's state (we pass a local copy), so we treat this as a pure mixer.
...
{
    // constants / bounds
    constexpr double MIN_R = 0.1;
    constexpr double MAX_R = 4.0;

    // 1) Normalize r_prev into [0,1] relative position within [MIN_R, MAX_R]
    double t;
    if (r_prev <= MIN_R)
        t = 0.0;
    else if (r_prev >= MAX_R)
        t = 1.0;
    else
        t = (r_prev - MIN_R) / (MAX_R - MIN_R);

    // 2) Map normalized double to a deterministic 64-bit integer
    //    Use full 64-bit range [0, 2^64-1]. Multiply by (2^64) and floor.
    //    Avoid using UINT64_MAX + 1.0 overflow; use long double for safety if needed.
    const long double denom = static_cast<long double>(std::numeric_limits<uint64_t>::max()) + 1.0L;
    uint64_t r_int = static_cast<uint64_t>(std::floor((long double)t * denom));

    // 3) Build a 64-bit seed combining r_int and Ci in a deterministic manner
    //    E.g., spread Ci into high and low halves to influence the 64-bit seed.
    uint64_t seed = r_int ^ ((static_cast<uint64_t>(Ci) << 32) | static_cast<uint64_t>(Ci));

    // 4) Mix seed using splitmix64 to get a well-distributed 64-bit output
    //    We use a local copy so splitmix64 acts as a pure mixer (no state mutation outside).
    uint64_t tmp = seed;
    uint64_t mixed64 = splitmix64(tmp);

    // 5) Convert mixed64 to a double in [0,1), then shift to [-0.5, 0.5)
    long double frac = static_cast<long double>(mixed64) / denom; // in [0,1)
    long double perturb_ld = frac - 0.5L;                         // in [-0.5,0.5)
    double perturb = static_cast<double>(perturb_ld);

    // 6) Apply perturbation to r_prev
    double r_new = r_prev + perturb;

    // 7) Reflect/clamp into [MIN_R, MAX_R] using mirror reflection
    if (r_new < MIN_R)
    {
        r_new = MIN_R + (MIN_R - r_new); // reflect below lower bound
    }
    if (r_new > MAX_R)
    {
        double overflow = r_new - MAX_R;
        r_new = MAX_R - overflow; // reflect above upper bound
    }
    // Safety saturation if reflection still left it out of bounds
    if (r_new < MIN_R)
        r_new = MIN_R;
    if (r_new > MAX_R)
        r_new = MAX_R;
...
void HybridParallelHash::finalize_and_fill_matrix(State final_state,
...
{
    out.clear();
    out.reserve(words_needed);
    State s = final_state;
    double r1 = r1_start, r2 = r2_start;
    for (size_t i = 0; i < words_needed; ++i)
    {
        s = li_dai_step(s, r1, r2);
        s = recombine(s, r1, r2);

        uint32_t xbits = float_to_uint31(s.x);
        uint32_t ybits = float_to_uint31(s.y);
        uint32_t zbits = float_to_uint31(s.z);

        uint32_t word = (xbits << 1) ^ ybits ^ (zbits << 16);
        out.push_back(word);

        r1 = regen_r_from_uint(r1, word ^ ybits);
        r2 = regen_r_from_uint(r2, word ^ zbits);
    }
}
